...
def plot_mask(mask, color="r", ax=None):
    ...
    for contour in contours:
        polygon = patches.Polygon(contour[:, [1,0]], fill=False, edgecolor=color, linewidth=1)
...
def plot_results(img, results, skip_box=False):
    matplotlib.rcParams['keymap.save'] = []  # remove default "s" key for saving
    ...
    instructions = (
        ...
        "0  → Incorrect - Save mask & Next image\n"
        "s  → Incorrect - Check with SAM2 & Next image\n"
        "n  → Incorremt - Next image"
    )
    plt.text(
        0.01, 0.99, instructions,
        transform=ax.transAxes,
        fontsize=12,
        verticalalignment='top',
    ...
    for i in range(nb_objects):
        ...
        def on_key(event):
            ...
            if key in ["1", "0", "s", "n"]:
                ...
                print("Wrong key! Press '1', '0', 's', or 'n'.")
